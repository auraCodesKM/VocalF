...
**Voice health screening platform with blockchain-verified reporting**
...
> Record your voice, get an AI-generated screening result, track your vocal health — secured on-chain.
...
VocalWell.ai is a full-stack voice health screening platform. The frontend is a Next.js 14 application that lets users record or upload voice samples, receive AI-generated pathology screening results from the backend, visualize their vocal health history, complete guided exercises, connect with doctors, and store tamper-proof reports on IPFS with on-chain verification via EduChain.

**Scope:** this screens for acoustic indicators of common voice pathologies — it is a research/portfolio project, not a certified diagnostic device, and its output is not medical advice.
...
- Full 3-page PDF report with acoustic measurements (download or view inline)
...
│   ├── config.ts                 # Backend API endpoint configuration
│   ├── supabase.ts               # Supabase client + auth functions
│   ├── auth-context.tsx          # Auth state provider (React context)
│   ├── exercises-context.tsx     # Exercise tracking state
│   ├── reportService.ts          # Persists analysis reports to Supabase
│   └── uploadUtils.ts            # IPFS/Pinata upload helpers
│
...
The `reports/` directory contains project documentation and a sample voice analysis report generated by the backend AI engine.
...
Backend reports and all 57 sample analyses: [auraCodesKM/vocalB](https://github.com/auraCodesKM/vocalB)
